...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/adwpc/xmobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/adwpc/xmobile/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/adwpc/xmobile/app": true}, nil
	}
	r, w := io.Pipe()
